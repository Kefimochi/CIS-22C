...
class AList<T> implements ListInterface<T> {
    ...
    */
    public void reverse() {
        assert (numberOfEntries >= 1) && !isEmpty();
        int counterFront = 1;
        int counterEnd = numberOfEntries;
        T tempData;

        if (numberOfEntries % 2 == 0) { 
            while (counterFront < counterEnd) {
                tempData = list[counterFront];
                replace(counterFront, list[counterEnd]);
                replace(counterEnd, tempData);
                counterFront++;
                counterEnd--;
            }
        } else {
    ...
    */
    public void cycle() {
        ...
        if (numberOfEntries == 1) {
            ...
            list[numberOfEntries] = firstItem; //setting last element to first data
        }
    }
